...
pub fn write_catalog(workspace: &Path, profile: &str, counts: &mut Counts) -> ToolResult<()> {
    let content = format!("profile = \"{profile}\"\nroot = \"docs\"\n");
    write_missing(workspace, "docs/catalog.toml", &content, counts)
}
