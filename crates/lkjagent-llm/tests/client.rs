...
fn action_call_spec_uses_compact_output_budget() {
    ...
    assert_eq!(spec.stop, vec!["</lkjagent_action_v2>".to_string()]);
}
...
fn local_stub_server_receives_request_and_returns_completion() -> TestResult<()> {
    let body = r#"{"choices":[{"message":{"content":"<lkjagent_action_v2></lkjagent_action_v2>"},"finish_reason":"stop"}],"usage":{"prompt_tokens":5,"completion_tokens":3},"prompt_cache_hit_tokens":4}"#;
    ...
    assert_eq!(
        request.body,
        "{\"model\":\"local-model\",\"messages\":[{\"role\":\"system\",\"content\":\"system\"}],\"max_tokens\":1024,\"temperature\":0.3,\"top_p\":0.9,\"reasoning_effort\":\"none\",\"stop\":[\"</lkjagent_action_v2>\"],\"stream\":false}"
    );
    Ok(())
}
...
fn length_with_closed_action_is_accepted() -> TestResult<()> {
    ...
    assert!(completion.content.contains("</lkjagent_action_v2>"));
    assert_eq!(
