...
fn action_call_spec_uses_compact_output_budget() {
    ...
    assert_eq!(spec.stop, vec!["</lkjagent_action>".to_string()]);
}
...
fn local_stub_server_receives_request_and_returns_completion() -> TestResult<()> {
    let body = r#"{"choices":[{"message":{"content":"<lkjagent_action></lkjagent_action>"},"finish_reason":"stop"}],"usage":{"prompt_tokens":5,"completion_tokens":3},"prompt_cache_hit_tokens":4}"#;
...
fn length_with_closed_action_is_accepted() -> TestResult<()> {
    ...
    assert!(completion.content.contains("</lkjagent_action>"));
    assert_eq!(
