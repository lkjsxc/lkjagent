...
#[test]
fn run_writes_first_config_starts_and_refuses_existing_lock() -> TestResult<()> {
    ...
    assert!(first.join("lkjagent.toml").exists());

    let ready = temp_data("ready")?;
    write_config(&ready)?;
    let started = run_cli(["--data", ready.to_string_lossy().as_ref(), "run"]);
    assert_eq!(started.code, 0);
    assert_eq!(started.stdout, "startup=ok\nshutdown=clean");

    let locked = temp_data("locked")?;
    write_config(&locked)?;
    let conn = open_store(&locked)?;
    lkjagent_store::state::take_lock(&conn, "other", "2026-01-01", "0")?;
