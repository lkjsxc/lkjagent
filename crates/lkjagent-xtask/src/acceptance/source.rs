...
const FACTS: &[Fact] = &[
    ...
    fact("X04", "crates/lkjagent-store/src/matter_control.rs", &["pub fn block_budget(", "'matter-blocked'", "'block','budget'", "lifecycle='blocked'", "pub fn resume_blocked(", "'owner-resume'", "accounted_tokens_in_budget_epoch", "effects_in_budget_epoch", "settlement_event_id=?1"]),
    fact("X04", "crates/lkjagent-app/src/public_loop.rs", &["MODEL_CALL_LIMIT", "TOKEN_BUDGET_LIMIT", "EFFECT_BUDGET_LIMIT", "exhausted {label} budget"]),
    fact("X04", "crates/lkjagent-store/tests/native_direct_loop.rs", &["exhausted_budget_is_visible_idempotent_and_not_idle", "effects_in_budget_epoch", "Some(\"blocked\")"]),
