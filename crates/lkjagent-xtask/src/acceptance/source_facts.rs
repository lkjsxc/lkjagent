...
#[rustfmt::skip]
pub(super) const FACTS: &[Fact] = &[
    fact("A01", "crates/lkjagent-xtask/tests/acceptance_negative.rs", &["acceptance_negative_rejects_claims_staleness_and_semantic_fakes", "acceptance-fake-pass.tsv", "let missing = verify", "has(&missing.errors, \"missing\")"]),
    fact("A02", "crates/lkjagent-xtask/tests/campaign_evidence.rs", &["daily_campaign_derives_only_from_source_bound_measured_rows", "manifest(&root, &directory, &source, 'b')", "source_hash(root, source, \"crates/lkjagent-xtask/src/acceptance.rs\")", "profile_plan_sha256"]),
    fact("A03", "crates/lkjagent-xtask/tests/acceptance_negative.rs", &["acceptance-scripted-semantic.tsv", "acceptance-placeholder-output.tsv"]),
    fact("A03", "crates/lkjagent-xtask/tests/acceptance_history.rs", &["legacy_editable_campaign_summary_derives_nothing", "let altered = body.replace(\"file_exact=true\", \"file_exact=false\")", "assert!(derived.is_empty())"]),
    fact("A04", "crates/lkjagent-xtask/tests/acceptance_negative.rs", &["acceptance_negative_rejects_short_quiet_and_secret_bytes", "shorter than 900", "quiet or missing"]),
    fact("A05", "crates/lkjagent-xtask/tests/acceptance_negative.rs", &["acceptance_negative_rejects_source_drift_untracked_and_history_secret", "has(&untracked.errors, \"untracked\")", "bytes suppressed"]),
    ...
    fact("F05", "crates/lkjagent-store/src/native-schema.sql", &["prior_bytes BLOB", "intended_bytes BLOB", "prior_mode INTEGER", "intended_mode INTEGER"]),
    fact("F05", "crates/lkjagent-app/tests/public_loop.rs", &["beta is the checked phrase", "mode()&0o7777,0o640", "role='agent' AND receipt IS NOT NULL", "Checked: notes/exact-base.txt@"]),
    fact("F08", "crates/lkjagent-app/tests/public_loop.rs", &["public_native_loop_exact_edit_second_matter_restart_and_stale_guard", "SELECT count(*) FROM matters WHERE lifecycle='closed'", "vec![(1,\"owner\".into()),(2,\"agent\".into()),(3,\"owner\".into()),(4,\"agent\".into())"]),
    fact("F06", "crates/lkjagent-effects/src/workspace_edit.rs", &["DurablePhase::Staged", "DurablePhase::Exchanged", "DurablePhase::Compensated", "DurablePhase::Settled", "cleanup_exact_edit"]),
    fact("F06", "crates/lkjagent-effects/tests/workspace_edit.rs", &["replacement_phases_are_durable_classified_and_cleanup_verified", "creation_races_compensate_without_overwriting_owner_values", "third-owner-value", "assert!(!stage.exists())"]),
    ...
    fact("R07", "crates/lkjagent-core/src/runtime_selector.rs", &["direct_tool_view_for_state", "RuntimePhase::Orient", "RuntimePhase::Respond"]),
    fact("R07", "crates/lkjagent-core/src/runtime_prompt_kernel.rs", &["selected_cell(decision, snapshot)?", "decision.tool_view.entries"]),
    fact("R07", "crates/lkjagent-core/tests/default_tool_view.rs", &["direct_view_fingerprints_are_stable_and_phase_specific", "selector_decisions_use_modify_review_and_respond_projections"]),
    fact("R08", "crates/lkjagent-core/tests/runtime_reducer_selector.rs", &["direct_transitions_do_not_add_model_review", "missing_fact_waits_on_owner_and_cooldown_has_typed_wake", "current_checks_respond_without_tools_and_guards_close", "Selection::Idle"]),
    ...
    fact("P05", "crates/lkjagent-app/src/journal_apply.rs", &["automatic_checks::reduce_committed_edit"]),
    ...
    fact("C03", "crates/lkjagent-core/src/runtime_context.rs", &["pub fn detect_contradictions(", "entry(normalized_body(&item.body))", "if bodies.len() < 2"]),
    fact("C03", "crates/lkjagent-core/tests/context_completion.rs", &["context_plan_suppresses_duplicate_clean_items", "assert_eq!(plan.included.len(), 2)"]),
    fact("C07", "crates/lkjagent-app/src/config.rs", &["fn prompt_max_context_tokens(", "LKJAGENT_PROMPT_CONTEXT_TOKENS", "prompt_context_tokens"]),
    fact("C07", "crates/lkjagent-app/src/public_loop.rs", &["compile_prompt(&decision,&snap,objective,&sources,&prompt_budgets(data)?)", "fn prompt_budgets(data:&Path)", "value.total_tokens=total"]),
    fact("C07", "crates/lkjagent-core/src/runtime_prompt_kernel.rs", &["pub struct PromptBudgets", "apply_budgets(&mut plan", "compiled prompt exceeds total token budget"]),
    ...
    fact("C05", "crates/lkjagent-app/src/history_context.rs", &["LIMIT 16", "project_ids", "wanted.is_disjoint"]),
    fact("C05", "crates/lkjagent-app/src/history_context.rs", &["exact_project_token_excludes_prefix_neighbor_history", "project-orbital"]),
    ...
    fact("X01", "crates/lkjagent-core/src/parse.rs", &["pub enum ParseFault", "parse_expected_for_decision"]),
    fact("X01", "crates/lkjagent-core/src/runtime_admission.rs", &["pub enum AdmissionStatus", "pub struct ToolAdmission"]),
    fact("X01", "crates/lkjagent-effects/src/workspace_edit_types.rs", &["pub enum EditError"]),
    fact("X01", "crates/lkjagent-llm/src/error.rs", &["pub enum EndpointFailure", "pub enum FaultClass"]),
    fact("X01", "crates/lkjagent-core/src/runtime_recovery.rs", &["pub enum FailureClass", "Parse", "OutputLimit", "Admission", "Endpoint", "Effect", "Check"]),
    fact("X01", "crates/lkjagent-core/src/runtime_operation.rs", &["pub enum BlockReason", "Stasis"]),
    ...
    fact("W01", "crates/lkjagent-app/src/workspace_root.rs", &["DEFAULT_WORKSPACE_ROOT", "workspace root must be separate from runtime data"]),
    fact("W01", "crates/lkjagent-app/tests/workspace_root.rs", &["resolves_default_relative_absolute_and_rejects_internal_roots", "environment_override_wins_in_an_isolated_process", "LKJAGENT_WORKSPACE_ROOT"]),
    fact("W01", "docker-compose.yml", &["${LKJAGENT_DATA_DIR:-./data}", "${LKJAGENT_WORKSPACE_DIR:-./workspace}", "target: /workspace"]),
    fact("W03", "crates/lkjagent-app/src/report_record.rs", &["artifacts/documents/{slug}/README.md", "journal_apply::prepare_mode"]),
    fact("W03", "crates/lkjagent-app/src/journal_apply.rs", &["intended:Some(&prepared.edit.intended_bytes)", "targets:&targets"]),
    fact("W03", "crates/lkjagent-app/tests/report_long_flow.rs", &["long_report_map_children_restart_receipt_and_replacement_are_exact", "fs::read_to_string(workspace.join(readme))", "effect_journal"]),
    fact("W05-deterministic", "crates/lkjagent-app/src/journal_dispatch.rs", &["selected_wall_time", "workspace_timezone", "canonical_path", "source_rows"]),
    fact("W05-deterministic", "crates/lkjagent-app/tests/journal_flow.rs", &["assert_ne!(date", "source_revision AS TEXT", "write_record"]),
    fact("W06-deterministic", "crates/lkjagent-app/src/memory_record.rs", &["semantic_slug", "knowledge/notes/{slug}.md", "source_rows"]),
    fact("W06-deterministic", "crates/lkjagent-app/tests/memory_flow.rs", &["memory_create_replace_retrieve_and_correction_are_revision_exact", "source_kind='memory'", "matches(\"project color is cobalt\").count(),1"]),
    fact("W08-deterministic", "crates/lkjagent-app/src/report_record.rs", &["artifacts/reports/{slug}.md", "artifacts/documents/{slug}/README.md", "report child does not match a pending current map"]),
    fact("W08-deterministic", "crates/lkjagent-app/tests/report_long_flow.rs", &["long_report_map_children_restart_receipt_and_replacement_are_exact", "managed-report-complete", "report-member/launch-plan/summary"]),
    fact("C04-deterministic", "crates/lkjagent-app/src/memory_context.rs", &["correction_keys", "strip_prefix(\"forget \")", "d.current_revision_id"]),
    fact("C04-deterministic", "crates/lkjagent-app/tests/memory_flow.rs", &["forget project-color:", "assert_ne!(revision,stale)", "ROGUE-FILESYSTEM-CONTENT"]),
    ...
    fact("T09", "crates/lkjagent-app/tests/tui_native.rs", &["terminal_pane_follow_reaches_newest_and_replaced_rows_disappear", "body 8"]),
    fact("T09", "crates/lkjagent-app/tests/tui_contract.rs", &["resize_search_shrink_and_overscroll_clamp_without_blank_windows", "assert!(!visible"]),
    fact("T10", "crates/lkjagent-app/src/tui_runtime.rs", &["load_older", "conversation_height", "TuiEvent::Snapshot"]),
    fact("T10", "crates/lkjagent-app/tests/tui_responsive.rs", &["input_remains_reducible_while_worker_endpoint_is_blocked", "Event::Resize", "Event::Paste(\"日本語\""]),
];
