...
pub fn evaluate(spec: &CheckSpec, files: &[FileFact], commands: &[CommandFact]) -> CheckResult {
    match spec {
        ...
        CheckSpec::LinksResolve { root } => result("links_resolve", has_root(files, root), root),
    }
}
...
fn has_root(files: &[FileFact], root: &str) -> bool {
    files.iter().any(|fact| fact.path.starts_with(root))
}

fn readme_coverage(files: &[FileFact], root: &str) -> CheckResult {
    let has_readme = files
        .iter()
        .any(|fact| fact.path == format!("{root}/README.md"));
    result("readme_coverage", has_readme, has_readme.to_string())
}
