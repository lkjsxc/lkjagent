...
fn blocked_matter_resumes_on_owner_send_unless_new_is_forced() -> TestResult<()> {
    ...
    let rows:(i64,i64,String)=connection.query_row("SELECT (SELECT count(*) FROM matters),(SELECT count(*) FROM owner_turns),lifecycle FROM matters WHERE id=?1",[matter],|r|Ok((r.get(0)?,r.get(1)?,r.get(2)?)))?;
    assert_eq!(rows, (1, 2, "open".into()));
    Ok(())
}
