...
fn public_native_loop_exact_edit_second_matter_restart_and_stale_guard()->TestResult {
 ...
 let mut endpoint=ScriptedEndpoint{outputs:vec![read("notes/exact-base.txt"),edit("notes/exact-base.txt","alpha is the known phrase","beta is the checked phrase"),final_message("Updated the exact phrase with current checks.")],index:0};
 for _ in 0..3 { public_loop::run_once(&data,&mut endpoint)?; }
 ...
 let second=cli::run(["--data",data_arg.as_ref(),"send","Update the second existing file exactly."])?; assert_ne!(identity(&first,"message="),identity(&second,"message=")); let mut endpoint=ScriptedEndpoint{outputs:vec![read("notes/second.txt"),edit("notes/second.txt","one is the second phrase","two is the second phrase"),final_message("Updated the second file with current checks.")],index:0}; for _ in 0..3{public_loop::run_once(&data,&mut endpoint)?;}
 assert_eq!(fs::read_to_string(workspace.join("notes/second.txt"))?,"two is the second phrase\n"); let c=Connection::open(&db)?; assert_eq!(scalar(&c,"SELECT count(*) FROM matters WHERE lifecycle='closed'")?,2); assert_eq!(roles(&c)?,vec![(1,"owner".into()),(2,"agent".into()),(3,"owner".into()),(4,"agent".into())]); assert_eq!(scalar(&c,"SELECT count(*) FROM checks WHERE current=1 AND passed=1")?,6); drop(c);
 let stale=cli::run(["--data",data_arg.as_ref(),"send","Make a revision-bound update."])?; assert_ne!(identity(&second,"message="),identity(&stale,"message=")); let mut endpoint=ScriptedEndpoint{outputs:vec![read("notes/exact-base.txt"),edit("notes/exact-base.txt","beta is the checked phrase","gamma must not win")],index:0}; public_loop::run_once(&data,&mut endpoint)?; fs::write(workspace.join("notes/exact-base.txt"),"owner bytes remain\n")?; let outcome=public_loop::run_once(&data,&mut endpoint)?; assert!(outcome.contains("fault:")); assert_eq!(fs::read_to_string(workspace.join("notes/exact-base.txt"))?,"owner bytes remain\n");
 let c=Connection::open(&db)?; assert_eq!(scalar(&c,"SELECT count(*) FROM effect_journal")?,2); assert_eq!(scalar(&c,"SELECT count(*) FROM state_cells WHERE CAST(namespace AS TEXT)='recovery' AND CAST(cell_key AS TEXT)='stale' AND status='active'")?,1); let status=cli::run(["--data",data_arg.as_ref(),"status"])?; assert!(status.contains("roots: data=")&&status.contains("workspace=")&&status.contains("unfinished:")&&status.contains("checks-ready=")); assert!(!status.contains("task:")); Ok(())
}
