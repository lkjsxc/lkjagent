...
fn public_native_loop_exact_edit_second_matter_restart_and_stale_guard()->TestResult {
 ...
 let mut endpoint=ScriptedEndpoint{outputs:vec!["<tool_call><tool>list_directory</tool><input><path>notes</path></input></tool".into(),read("notes/missing.txt"),read("notes/exact-base.txt"),edit("notes/exact-base.txt","alpha is the known phrase","beta is the checked phrase"),final_message("Updated the exact phrase with current checks.")],index:0};
 for _ in 0..5 { public_loop::run_once(&data,&mut endpoint)?; }
