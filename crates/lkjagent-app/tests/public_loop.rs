...
fn public_native_loop_exact_edit_second_matter_restart_and_stale_guard()->TestResult {
 ...
 let mut endpoint=ScriptedEndpoint{outputs:vec!["<tool_call><tool>list_directory</tool><input><path>notes</path></input></tool".into(),read("notes/missing.txt"),read("notes/exact-base.txt"),edit("notes/exact-base.txt","alpha is the known phrase","beta is the checked phrase"),final_message("I will update the exact phrase."),final_message("Updated the exact phrase with current checks.")],index:0};
 for _ in 0..6 { public_loop::run_once(&data,&mut endpoint)?; }
 ...
 let second=cli::run(["--data",data_arg.as_ref(),"send","Re-read the checked file, make no changes, and report it."])?; assert_ne!(identity(&first,"message="),identity(&second,"message=")); let mut endpoint=ScriptedEndpoint{outputs:vec![read("notes/exact-base.txt"),final_message("Ready to report the checked phrase."),"<final>".into()],index:0}; for _ in 0..3{public_loop::run_once(&data,&mut endpoint)?;}
 assert_eq!(fs::read_to_string(workspace.join("notes/second.txt"))?,"one is the second phrase\n"); let c=Connection::open(&db)?; assert_eq!(scalar(&c,"SELECT count(*) FROM matters WHERE lifecycle='closed'")?,2); assert_eq!(roles(&c)?,vec![(1,"owner".into()),(2,"agent".into()),(3,"owner".into()),(4,"agent".into())]); assert_eq!(scalar(&c,"SELECT count(*) FROM checks WHERE current=1 AND passed=1")?,6); assert_eq!(scalar(&c,"SELECT count(*) FROM context_items WHERE source_kind='conversation-history'")?,0);assert_eq!(scalar(&c,"SELECT count(*) FROM runtime_decisions WHERE instr(CAST(rendered_frame AS TEXT),'{&quot;')>0")?,0); drop(c);
