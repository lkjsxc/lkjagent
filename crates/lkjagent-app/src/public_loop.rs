...
pub fn run_once(data:&Path,endpoint:&mut dyn Endpoint)->R<String>{
 ...
 let parsed=match lkjagent_core::parse::parse_expected_for_decision(&decision,&answer.content){Ok(v)=>v,Err(x)=>return fault(&mut store,&did,&m.id,if format!("{x:?}").contains("UnknownTool"){ModelFaultKind::Hidden}else{ModelFaultKind::Malformed},&format!("{x:?}"))};
 match parsed { ParsedOutput::Action(a)=>dispatch(data,&db,&mut store,&m.id,&did,&decision,&a.tool,&a.params,&answer.content), ParsedOutput::Message(body)=>close(&mut store,&m.id,&did,&body), _=>fault(&mut store,&did,&m.id,ModelFaultKind::Malformed,"wrong compact envelope") }
}
...
fn dispatch(data:&Path,db:&Path,store:&mut NativeStore,matter:&str,did:&str,d:&RuntimeDecision,tool:&str,args:&[(String,String)],raw:&str)->R<String>{
 ...
 match effect.0.as_str(){
  "workspace.read"=>{let page=ws.read_file(path,get("offset").and_then(|v|v.parse().ok()).unwrap_or(1),get("count").and_then(|v|v.parse().ok()).unwrap_or(200)).map_err(e)?; let observed=ws.observe_edit_target(path).map_err(e)?; let lkjagent_effects::workspace_edit::ObservedTarget::Present(file)=observed else{return Err("read target absent".into())}; let revision=file.revision.clone(); let body=json!({"path":path,"revision":revision,"bytes":String::from_utf8_lossy(&file.bytes),"page":format!("{page:?}")}).to_string(); let outcome=direct(store,matter,did,raw,entry,DirectTool::Read,b"source",b"current",body.as_bytes(),&sha(&file.bytes))?; if get("complete")==Some("true")&&!store.reuse_checked_revision(matter,did,path.as_bytes(),&file.revision).map_err(e)?{return Err("read-only completion requires current passed checks for this exact revision".into())} Ok(outcome)},
...
#[rustfmt::skip]
fn source_context(cells:&[lkjagent_store::direct_transactions::CellRow])->Vec<ContextItem>{cells.iter().filter(|c|c.namespace==b"source").filter_map(|c|serde_json::from_slice::<Value>(&c.payload).ok()).map(|v|context("source","workspace-source",v["body_ref"].as_str().unwrap_or("").into(),TrustClass::Measured,"workspace",v["revision"].as_str().unwrap_or(""))).collect()}
#[rustfmt::skip]
fn source_revision(store:&NativeStore,path:&str)->R<String>{store.restart_projection().map_err(e)?.cells.into_iter().find(|c|c.namespace==b"source").and_then(|c|serde_json::from_slice::<Value>(&c.payload).ok()).and_then(|outer|serde_json::from_str::<Value>(outer["body_ref"].as_str()?).ok()).filter(|inner|inner["path"]==path).and_then(|inner|inner["revision"].as_str().map(str::to_string)).ok_or_else(||"no current revision-bound source for edit".into())}
#[rustfmt::skip]
