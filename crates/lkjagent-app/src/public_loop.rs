...
pub fn run_once(data:&Path,endpoint:&mut dyn Endpoint)->R<String>{
 ...
 let objective=context("objective","objective",String::from_utf8_lossy(&m.objective).into_owned(),TrustClass::Owner,"owner",&m.id); let mut sources=source_context(&p.cells);sources.extend(history_context(&db,&m.id)?);let items=std::iter::once(objective.clone()).chain(sources.iter().cloned()).collect::<Vec<_>>();let compiled=compile_prompt(&decision,&snap,objective,&sources,&PromptBudgets::default()).map_err(|x|fault(&mut store,&did,&m.id,ModelFaultKind::Malformed,&x).unwrap_or(x))?; decision.context_frame_fingerprint=compiled.prompt.fingerprint.clone();
...
fn dispatch(data:&Path,db:&Path,store:&mut NativeStore,matter:&str,did:&str,d:&RuntimeDecision,tool:&str,args:&[(String,String)],raw:&str)->R<String>{
 ...
 match effect.0.as_str(){
  "workspace.read"=>{let page=ws.read_file(path,get("offset").and_then(|v|v.parse().ok()).unwrap_or(1),get("count").and_then(|v|v.parse().ok()).unwrap_or(200)).map_err(e)?; let observed=ws.observe_edit_target(path).map_err(e)?; let lkjagent_effects::workspace_edit::ObservedTarget::Present(file)=observed else{return Err("read target absent".into())}; let revision=file.revision.clone(); let body=json!({"path":path,"revision":revision,"bytes":String::from_utf8_lossy(&file.bytes),"page":format!("{page:?}")}).to_string(); let outcome=direct(store,matter,did,raw,entry,DirectTool::Read,b"source",b"current",body.as_bytes(),&sha(&file.bytes))?; if get("complete")==Some("true"){let _=store.reuse_checked_revision(matter,did,path.as_bytes(),&file.revision).map_err(e)?;} Ok(outcome)},
...
pub fn doctor(data:&Path,json_output:bool)->R<String>{fs::create_dir_all(data).map_err(e)?;let db=data.join("lkjagent.sqlite3");let store=NativeStore::open(&db).map_err(e)?;let p=store.restart_projection().map_err(e)?;let c=Connection::open(&db).map_err(e)?;
 let matters:i64=c.query_row("SELECT count(*) FROM matters",[],|r|r.get(0)).map_err(e)?;let (workspace,present)=crate::config::workspace_state(data)?;let endpoint=crate::config::endpoint_state(data);let prompt=crate::config::prompt_max_context_tokens(data)?.unwrap_or(0);let live=crate::config::live_campaign_seconds(data)?.unwrap_or(0);
 if json_output{return serde_json::to_string(&json!({"data_root":data,"workspace_root":workspace,"workspace_present":present,"endpoint":endpoint,"schema":"native-18","table_count":lkjagent_store::native_schema::NATIVE_TABLES.len(),"missing_tables":[],"matters":matters,"unfinished_decisions":p.decisions.len(),"prompt_max_context_tokens":prompt,"live_campaign_seconds":live})).map_err(e)}
 Ok(format!("schema=native-18 tables={} missing=0\nroots: data={} workspace={} workspace_present={present}\nendpoint: {endpoint}\nmatters={matters} unfinished_decisions={} prompt_cap={prompt} live_seconds={live}",lkjagent_store::native_schema::NATIVE_TABLES.len(),data.display(),workspace.display(),p.decisions.len()))}
