...
fn dispatch(data:&Path,db:&Path,store:&mut NativeStore,matter:&str,did:&str,d:&RuntimeDecision,tool:&str,args:&[(String,String)],raw:&str)->R<String>{
 ...
 match effect.0.as_str(){
  ...
  "workspace.list"=>{let listing=ws.list_directory(path,get("offset").and_then(|v|v.parse().ok()).unwrap_or(0),get("count").and_then(|v|v.parse().ok()).unwrap_or(20)).map_err(e)?;if let Some(target)=managed_path(db){if let Ok(page)=ws.read_file(&target,1,200){let lines=page.lines.iter().map(|line|json!({"number":line.number,"text":&line.text})).collect::<Vec<_>>();let out=json!({"path":&target,"revision":&page.revision,"lines":lines,"total_lines":page.total_lines,"truncated":page.truncated,"next_line":page.next_line,"final_newline":page.final_newline,"listing_path":path,"listing":format!("{listing:?}")}).to_string();let outcome=direct(store,matter,did,raw,entry,DirectTool::List,b"source",b"current",out.as_bytes(),page.revision.as_bytes())?;if get("complete")==Some("true"){let _=store.reuse_checked_revision(matter,did,target.as_bytes(),&page.revision).map_err(e)?;}return Ok(outcome)}}let out=json!({"path":path,"listing":format!("{listing:?}")}).to_string();let fp=sha(out.as_bytes());direct(store,matter,did,raw,entry,DirectTool::List,b"observation",b"current",out.as_bytes(),&fp)},
