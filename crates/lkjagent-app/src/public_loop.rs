...
pub fn run_once(data:&Path,endpoint:&mut dyn Endpoint)->R<String>{
 ...
 let seq=store.next_event_sequence(&m.id).map_err(e)?; let did=id("decision",format!("{}:{seq}:{}",m.id,spec.operation_key).as_bytes()); let devent=id("selected",did.as_bytes()); let mut decision=runtime_decision(&did,&m.id,&snap,&spec)?; let specs=specs(&decision,&spec,&bound)?;
 ...
 let objective=context("objective","objective",String::from_utf8_lossy(&m.objective).into_owned(),TrustClass::Owner,"owner",&m.id); let mut sources=source_context(&p.cells);sources.extend(crate::history_context::load(&db,&m.id,&m.objective)?);let items=std::iter::once(objective.clone()).chain(sources.iter().cloned()).collect::<Vec<_>>();let compiled=compile_prompt(&decision,&snap,objective,&sources,&prompt_budgets(data)?).map_err(|x|fault(&mut store,&did,&m.id,ModelFaultKind::Malformed,&x).unwrap_or(x))?; decision.context_frame_fingerprint=compiled.prompt.fingerprint.clone();
...
fn dispatch(data:&Path,db:&Path,store:&mut NativeStore,matter:&str,did:&str,d:&RuntimeDecision,tool:&str,args:&[(String,String)],raw:&str)->R<String>{
 ...
 if effect.0=="workspace.record.journal"{return crate::journal_dispatch::dispatch(data,db,store,matter,did,entry,args,raw)}let root=crate::config::workspace_root(data)?;let root=crate::workspace_root::open(&root)?;let ws=OpenedWorkspace::open(&root).map_err(e)?;let path=get("path").ok_or("path missing")?;
 match effect.0.as_str(){
  "workspace.read"=>{let page=ws.read_file(path,get("offset").and_then(|v|v.parse().ok()).unwrap_or(1),get("count").and_then(|v|v.parse().ok()).unwrap_or(200)).map_err(e)?; let lines=page.lines.iter().map(|line|json!({"number":line.number,"text":&line.text})).collect::<Vec<_>>();let context=page.lines.iter().map(|line|format!("{}: {}",line.number,line.text)).collect::<Vec<_>>().join("\n"); let body=json!({"path":&page.path,"revision":&page.revision,"lines":lines,"total_lines":page.total_lines,"truncated":page.truncated,"next_line":page.next_line,"final_newline":page.final_newline}).to_string(); let outcome=direct(store,matter,did,raw,entry,DirectTool::Read,b"source",b"current",page.path.as_bytes(),body.as_bytes(),context.as_bytes(),page.revision.as_bytes())?; if get("complete")==Some("true"){let _=store.reuse_checked_revision(matter,did,path.as_bytes(),&page.revision).map_err(e)?;} Ok(outcome)},
  "workspace.list"=>{let listing=ws.list_directory(path,get("offset").and_then(|v|v.parse().ok()).unwrap_or(0),get("count").and_then(|v|v.parse().ok()).unwrap_or(20)).map_err(e)?;if let Some(target)=managed_path(db){if let Ok(page)=ws.read_file(&target,1,200){let lines=page.lines.iter().map(|line|json!({"number":line.number,"text":&line.text})).collect::<Vec<_>>();let context=page.lines.iter().map(|line|format!("{}: {}",line.number,line.text)).collect::<Vec<_>>().join("\n");let out=json!({"path":&target,"revision":&page.revision,"lines":lines,"total_lines":page.total_lines,"truncated":page.truncated,"next_line":page.next_line,"final_newline":page.final_newline,"listing_path":path,"listing":format!("{listing:?}")}).to_string();let outcome=direct(store,matter,did,raw,entry,DirectTool::List,b"source",b"current",target.as_bytes(),out.as_bytes(),context.as_bytes(),page.revision.as_bytes())?;if get("complete")==Some("true"){let _=store.reuse_checked_revision(matter,did,target.as_bytes(),&page.revision).map_err(e)?;}return Ok(outcome)}}let out=json!({"path":path,"listing":format!("{listing:?}")}).to_string();let fp=sha(out.as_bytes());let context=format!("directory {path}\n{listing:?}");direct(store,matter,did,raw,entry,DirectTool::List,b"observation",b"current",path.as_bytes(),out.as_bytes(),context.as_bytes(),&fp)},
