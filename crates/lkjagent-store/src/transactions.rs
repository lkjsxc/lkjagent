...
impl NativeStore {
    ...
    #[rustfmt::skip]
    pub fn reuse_checked_revision(&mut self, matter: &str, path: &[u8], revision: &str) -> StoreResult<bool> {
        self.atomic(|tx| {
            let source: Option<(String,String)> = tx.query_row("SELECT rd.matter_id,(SELECT o.decision_id FROM state_cells s JOIN observations o ON o.event_id=s.source_event_id WHERE s.matter_id=?3 AND CAST(s.namespace AS TEXT)='source' AND CAST(s.cell_key AS TEXT)='current' AND s.status='active') FROM workspace_documents d JOIN workspace_revisions r ON r.id=d.current_revision_id JOIN effect_journal e ON e.id=r.effect_id JOIN runtime_decisions rd ON rd.id=e.decision_id JOIN matters m ON m.id=rd.matter_id WHERE d.current_path=?1 AND lower(hex(r.sha256))=?2 AND e.status='settled' AND m.lifecycle='closed'", params![path,revision,matter], |r| Ok((r.get(0)?,r.get(1)?))).optional()?;
            let Some((source,decision)) = source else { return Ok(false); };
            let count = tx.execute("INSERT INTO obligations(id,matter_id,predicate_kind,predicate_payload,required,status) SELECT ?1||'/continuity/'||o.id,?1,o.predicate_kind,o.predicate_payload,1,'open' FROM obligations o JOIN checks c ON c.id=o.current_check_id WHERE o.matter_id=?2 AND o.required=1 AND o.status='passed' AND c.current=1 AND c.passed=1 ON CONFLICT(id) DO NOTHING", params![matter,source])?;
            tx.execute("INSERT INTO checks(id,matter_id,obligation_id,decision_id,kind,parameters,current,passed,measured,evidence_fingerprint,source_revision,checked_event_id) SELECT ?1||'/continuity/'||c.id,?1,?1||'/continuity/'||o.id,?3,c.kind,c.parameters,1,1,c.measured,c.evidence_fingerprint,c.source_revision,rd.settlement_event_id FROM obligations o JOIN checks c ON c.id=o.current_check_id JOIN runtime_decisions rd ON rd.id=?3 WHERE o.matter_id=?2 AND o.required=1 AND o.status='passed' AND c.current=1 AND c.passed=1 ON CONFLICT(id) DO NOTHING", params![matter,source,decision])?;
            tx.execute("UPDATE obligations SET status='passed',current_check_id=(SELECT c.id FROM checks c WHERE c.obligation_id=obligations.id AND c.current=1) WHERE matter_id=?1 AND status='open' AND id LIKE ?1||'/continuity/%'", [matter])?;
            tx.execute("INSERT INTO state_cells(matter_id,namespace,cell_key,payload,status,source_event_id,fingerprint) SELECT ?1,'check','current-passed',CAST('{}' AS BLOB),'active',settlement_event_id,CAST(?3 AS BLOB) FROM runtime_decisions WHERE id=?2 ON CONFLICT(matter_id,namespace,cell_key) DO UPDATE SET status='active',source_event_id=excluded.source_event_id,fingerprint=excluded.fingerprint", params![matter,decision,revision])?; tx.execute("UPDATE state_cells SET status='suppressed' WHERE matter_id=?1 AND CAST(namespace AS TEXT)='recovery' AND status='active'", [matter])?;
            Ok(count > 0)
        })
    }
