...
impl NativeStore {
    ...
    pub fn reuse_checked_revision(&mut self, matter: &str, decision: &str, path: &[u8], revision: &str) -> StoreResult<bool> {
        self.atomic(|tx| {
            let source: Option<String> = tx.query_row("SELECT rd.matter_id FROM workspace_documents d JOIN workspace_revisions r ON r.id=d.current_revision_id JOIN effect_journal e ON e.id=r.effect_id JOIN runtime_decisions rd ON rd.id=e.decision_id JOIN matters m ON m.id=rd.matter_id WHERE d.current_path=?1 AND lower(hex(r.sha256))=?2 AND e.status='settled' AND m.lifecycle='closed'", params![path,revision], |r| r.get(0)).optional()?;
