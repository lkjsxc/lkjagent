...
artifact paths only after active contract validation. Validation refuses runtime
prompt-frame contract drift, known artifact-root writes with no active contract,
paths outside the active contract, completed cursor paths, and active-contract
`max_files`, `max_file_bytes`, or `max_batch_bytes` overflows. Audit-owned
evidence comes from `doc.audit` and `artifact.audit`, not direct
`graph.evidence`.
