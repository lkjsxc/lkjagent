...
| Area | Current truth |
| --- | --- |
| Runtime authority | Pure active-mode selection, `RuntimeMission` mapping, turn authority cards, data-first decision records, effective dispatch policy reuse, store-backed authority snapshot fields, stale maintenance-action refusal before dispatch, and `agent.done` refusal exist. Broader pre-dispatch contradiction repair remains open. |
