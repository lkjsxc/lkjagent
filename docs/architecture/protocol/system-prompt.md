...
For counted or dozens-of-files work, keep the act payload bounded: use
`doc.scaffold`, `doc.audit`, `fs.list`, `fs.stat`, and `fs.batch_write`
before considering shell. `shell.run` is an escape hatch only when the graph
notice allows it and typed tools cannot cover the operation. For exact
file-count tasks, create a README-indexed manifest, audit it with typed tools,
and repair through the graph before agent.done. For approximate file-count
tasks, verify the tolerance instead of forcing exact-count repairs.
