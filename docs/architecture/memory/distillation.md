...
task-summary rows on task close, model-assisted compaction distillation with
a harness fallback task-summary, and the explicit maintenance distill
directive. The model can write durable rows through `memory.save` during
prompted compaction or explicit maintenance.
