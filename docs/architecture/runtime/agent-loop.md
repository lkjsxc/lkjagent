...
directory indexes, first and last main files, and part ledger, records graph
evidence, saves a task summary, and closes the task without asking the
endpoint to repeat the same bulk generation. The recorded evidence includes
the `structured-output` path, target file count, index file count, design
memo count, main file count, acceptance-audit status, first and last main
