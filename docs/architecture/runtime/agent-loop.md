...
a per-part role ledger to the main index, verifies the requested count,
...
includes the `structured-output` path, target file count, design memo count,
main file count, and `verification=ok`.
