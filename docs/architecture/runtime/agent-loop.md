...
directory indexes, docs coverage map, all design memo sections, all main-file
sections, and part ledger, records graph evidence, saves a task summary, and
closes the task without asking the endpoint to repeat the same bulk
...
coverage-map status, acceptance-audit status, `section_scope=all`, required
